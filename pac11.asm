...
Const_TOTAL_DOTS    byte                55                 ; Total number of dots on the map
...
ska
                    lda                 gh1_cdir            
leftright           cmp                 Const_LEFT
                    beq                 lft_ck
                    cmp                 Const_RIGHT
                    beq                 rt_ck
udir                cmp                 #$5a                ; z pressed?
                    beq                 warpL
nextck1             cmp                 #$57                ; up pressed?
                    beq                 warpR
                    jmp                 nextck2
warpL               ldx                 Const_DOWN          ;Temp store 'D' down value
                    jmp                 warp_tnnl
warpR               ldx                 Const_UP            ;Temp store 'U' up value
...
ck_leftside         lda                 cdir
                    ...
                    lda                 #39
                    sta                 gx
                    lda                 #1                      ; Set going through
                    sta                 warp                    ; Warp wall flag
@end                rts
...
;*****************************************************
Can_Move_This_Dir?                                          ; Check if pac-clone hits a wall_dot
...
ck_3                cmp                 Const_LEFT         ; LEFT
                    bne                 ck_4
                    lda                 warp               ; Coming through
                    cmp                 #1                 ; one of the warp tunnels?
                    bne                 subtox             ; do not move, Fixes bug

                    lda                 #0                 ; Exiting warp tunnel
                    sta                 warp               ; 
                    lda                 g$,x               ; Coninue in same dir
                    jmp                 ck_4
subtox              dec                 gx                 ; LEFT, Decrease X
ck_4                cmp                 Const_RIGHT        ; 
                    bne                 skip
                    lda                 warp               ; Going through Warp tunnel?
                    cmp                 #1                 ; do not move, Fixes bug
                    bne                 addtogx
                    lda                 #0                 ; Exiting warp tunnel
                    sta                 warp
                    jmp                 skip
addtogx             inc                 gx                 ; RIGHT, Increase X
skip
